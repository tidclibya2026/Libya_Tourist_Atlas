...
R=Path(__file__).resolve().parents[1];PORT=0;bad=[];errors=[];req=[];fail=[]
def out(k,v):print(f'{k} = {"PASS" if v else "FAIL"}');fail.append(k) if not v else None
class H(http.server.SimpleHTTPRequestHandler):
 def log_message(self,*a):pass
os.chdir(R);srv=socketserver.TCPServer(('127.0.0.1',0),H);PORT=srv.server_address[1];threading.Thread(target=srv.serve_forever,daemon=True).start()
try:
 from playwright.sync_api import sync_playwright
 with sync_playwright() as p:
  b=p.chromium.launch(headless=True);page=b.new_page();page.add_init_script('window.__ATLAS_OFFLINE__=true');page.on('console',lambda m:errors.append(m.text) if m.type=='error' else None);page.on('pageerror',lambda e:errors.append(str(e)));page.on('response',lambda r:bad.append(r.url) if r.status==404 else None);page.on('request',lambda r:req.append(r.url));page.goto(f'http://127.0.0.1:{PORT}/index.html',wait_until='domcontentloaded',timeout=60000);page.wait_for_timeout(2000);toolreq=[];tool=b.new_page();tool.on('request',lambda r:toolreq.append(r.url));tool.goto(f'http://127.0.0.1:{PORT}/tools/image-review-phase-2/index.html',wait_until='networkidle');tool.select_option('#batch','world-heritage-akakus-old-tripoli');tool.wait_for_timeout(1000)
