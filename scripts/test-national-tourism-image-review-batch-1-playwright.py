...
R=Path(__file__).resolve().parents[1];PORT=8765;bad=[];errors=[];req=[];fail=[]
...
try:
 ...
 with sync_playwright() as p:
  b=p.chromium.launch(headless=True);page=b.new_page();page.on('console',lambda m:errors.append(m.text) if m.type=='error' else None);page.on('pageerror',lambda e:errors.append(str(e)));page.on('response',lambda r:bad.append(r.url) if r.status==404 else None);page.on('request',lambda r:req.append(r.url));page.goto(f'http://127.0.0.1:{PORT}/index.html',wait_until='domcontentloaded',timeout=60000);page.wait_for_timeout(2000);toolreq=[];tool=b.new_page();tool.on('request',lambda r:toolreq.append(r.url));tool.goto(f'http://127.0.0.1:{PORT}/tools/image-review-phase-2/index.html',wait_until='networkidle');tool.select_option('#batch','world-heritage-akakus-old-tripoli');tool.wait_for_timeout(1000)
