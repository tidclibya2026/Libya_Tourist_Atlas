...
const geo=JSON.parse(fs.readFileSync('data/layers/world-heritage.geojson','utf8'));const backup=JSON.parse(execFileSync('git',['show','HEAD:data/layers/world-heritage.geojson'],{encoding:'utf8'}));
...
const ids=new Set(); const failures=[]; let linked=0,without=0; const paths=new Set();const bm=new Map(backup.features.filter(f=>f.properties.parent_site_id==='WH-MAIN-001').map(f=>[f.properties.id,JSON.stringify(f)]));
